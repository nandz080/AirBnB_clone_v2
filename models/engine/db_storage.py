#!/usr/bin/python3
""" Module for new class for sqlAlchemy """
from os import getenv
from sqlalchemy.orm import sessionmaker, scoped_session
from sqlalchemy import (create_engine)
from sqlalchemy.ext.declarative import declarative_base
from models.base_model import Base
from models.state import State
from models.city import City
from models.user import User
from models.place import Place
from models.review import Review
from models.amenity import Amenity


class DBStorage:
    """ Module creates tables in environmental"""
    __engine = None
    __session = None

    def __init__(self):
        user = getenv("HBNB_MYSQL_USER")
        passwd = getenv("HBNB_MYSQL_PWD")
        db = getenv("HBNB_MYSQL_DB")
        host = getenv("HBNB_MYSQL_HOST")
        env = getenv("HBNB_ENV")

        self.__engine = create_engine('mysql+mysqldb://{}:{}@{}/{}'
                                      .format(user, passwd, host, db),
                                      pool_pre_ping=True)

        if env == "test":
            Base.metadata.drop_all(self.__engine)

    def all(self, cls=None):
        """method that returns a dictionary
        Return:
            returns a dictionary of __object
        """
        dic = {}
        if cls:
            if type(cls) is str:
                cls = eval(cls)
            query = self.__session.query(cls)
            for elemnt in query:
                key = "{}.{}".format(type(elemnt).__name__, elemnt.id)
                dic[key] = elemnt
        else:
            list_ = [State, City, User, Place, Review, Amenity]
            for class_ in list_:
                query = self.__session.query(class_)
                for elemnt in query:
                    key = "{}.{}".format(type(elemnt).__name__, elemnt.id)
                    dic[key] = elemnt
        return (dic)

    def new(self, obj):
        """method that adds a new element in the table
        """
        self.__session.add(obj)

    def save(self):
        """method that saves changes"""
        self.__session.commit()

    def delete(self, obj=None):
        """method that deletes an element in the table
        """
        if obj:
            self.session.delete(obj)

    def reload(self):
        """session configuration method
        """
        Base.metadata.create_all(self.__engine)
        s = sessionmaker(bind=self.__engine, expire_on_commit=False)
        Session = scoped_session(sec)
        self.__session = Session()

    def close(self):
        """ method that calls remove()"""
        self.__session.close()
